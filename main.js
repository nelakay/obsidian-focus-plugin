...
function serializeTask(task, includeCompletedAt = false) {
  ...
  const recurrencePart = task.recurrence ? ` \u{1F501} ${task.recurrence.type}:${task.recurrence.interval}${task.recurrence.type === "weeks" && task.recurrence.dayOfWeek != null ? ":" + task.recurrence.dayOfWeek : ""}${task.recurrence.type === "months" && task.recurrence.dayOfMonth != null ? ":" + task.recurrence.dayOfMonth : ""}` : "";
...
function computeNextRecurrenceDate(recurrence, fromDate) {
  const base = fromDate ? /* @__PURE__ */ new Date(fromDate + "T00:00:00") : /* @__PURE__ */ new Date();
  ...
  async archiveOrRestoreTask(task, section, data) {
    ...
    if (task.completed) {
      ...
      if (task.recurrence) {
        ...
        new import_obsidian.Notice(`Next occurrence created for ${nextDoDate}`);
      }
    } else {
...
var AddTaskModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(recurrenceContainer).setName("Repeat").setDesc("Make this a recurring task").addDropdown((dropdown) => {
      dropdown.addOption("none", "Never").addOption("days", "Every N days").addOption("weeks", "Every N weeks").addOption("months", "Every N months").setValue(this.recurrenceType).onChange((value) => {
    ...
    new import_obsidian2.Setting(recurrenceDetailsEl).setName("Every").addText((text) => {
    ...
    const dayOfMonthSetting = new import_obsidian2.Setting(recurrenceDetailsEl).setName("Day of month").addText((text) => {
      text.inputEl.type = "number";
      text.inputEl.min = "1";
      text.inputEl.max = "31";
      text.inputEl.style.width = "60px";
...
var FocusSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian5.Setting(containerEl).setName("Cloud sync").setHeading();
    new import_obsidian5.Setting(containerEl).setName("Enable cloud sync").setDesc("Sync tasks with the Focus PWA via Supabase").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.cloudSyncEnabled).onChange(async (value) => {
    ...
    if (this.plugin.settings.cloudSyncEnabled) {
      new import_obsidian5.Setting(containerEl).setName("Email").setDesc("Supabase auth email").addText(
        (text) => text.setPlaceholder("you@example.com").setValue(this.plugin.settings.supabaseEmail).onChange(async (value) => {
          this.plugin.settings.supabaseEmail = value;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian5.Setting(containerEl).setName("Password").setDesc("Supabase auth password").addText((text) => {
        text.setPlaceholder("\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022").setValue(this.plugin.settings.supabasePassword).onChange(async (value) => {
          this.plugin.settings.supabasePassword = value;
          await this.plugin.saveSettings();
        });
        text.inputEl.type = "password";
      });
      new import_obsidian5.Setting(containerEl).setName("Sync now").setDesc("Sign in and sync with the Focus PWA").addButton(
        (button) => button.setButtonText("Connect & sync").setCta().onClick(async () => {
          button.setButtonText("Syncing...");
          button.setDisabled(true);
...
var SUPABASE_URL = "https://mhieisdpfojkxebawtfj.supabase.co";
var SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1oaWVpc2RwZm9qa3hlYmF3dGZqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA2OTMyMTIsImV4cCI6MjA4NjI2OTIxMn0.FtCtTdpDvQ11n8RlTG4TEitb9SOeqWdySCfSQnjltas";
var client = null;
...
function initSupabase() {
  client = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
    auth: {
      ...
      // Obsidian plugin manages its own persistence
      autoRefreshToken: true
    }
  });
  return client;
...
function getUserId() {
  var _a, _b;
  return (_b = (_a = currentSession == null ? void 0 : currentSession.user) == null ? void 0 : _a.id) != null ? _b : null;
}
function destroySupabase() {
  client = null;
  currentSession = null;
}
...
function dbTaskToLocal(row) {
  var _a, _b, _c, _d, _e, _f, _g;
  const recurrence = row.recurrence_type && row.recurrence_interval ? {
    type: row.recurrence_type,
    interval: row.recurrence_interval,
    dayOfWeek: (_a = row.recurrence_day_of_week) != null ? _a : void 0,
    dayOfMonth: (_b = row.recurrence_day_of_month) != null ? _b : void 0
  } : void 0;
  return {
    id: row.id,
    title: row.title,
    completed: row.completed,
    completedAt: (_c = row.completed_at) != null ? _c : void 0,
    section: row.section,
    goalId: (_d = row.goal_id) != null ? _d : void 0,
    url: (_e = row.url) != null ? _e : void 0,
    doDate: (_f = row.do_date) != null ? _f : void 0,
    doTime: (_g = row.do_time) != null ? _g : void 0,
    recurrence
  };
}
function localTaskToDbFields(task, userId) {
  ...
  return {
    ...
    completed_at: (_a = task.completedAt) != null ? _a : null,
    section: task.section,
    goal_id: (_b = task.goalId) != null ? _b : null,
    url: (_c = task.url) != null ? _c : null,
    do_date: (_d = task.doDate) != null ? _d : null,
    do_time: (_e = task.doTime) != null ? _e : null,
    recurrence_type: (_g = (_f = task.recurrence) == null ? void 0 : _f.type) != null ? _g : null,
    recurrence_interval: (_i = (_h = task.recurrence) == null ? void 0 : _h.interval) != null ? _i : null,
    recurrence_day_of_week: (_k = (_j = task.recurrence) == null ? void 0 : _j.dayOfWeek) != null ? _k : null,
    recurrence_day_of_month: (_m = (_l = task.recurrence) == null ? void 0 : _l.dayOfMonth) != null ? _m : null,
    sort_order: 0,
    archived_month: null
  };
}
async function pullFromRemote() {
  ...
  const allDbTasks = tasksRes.data || [];
  ...
  const goals = dbGoals.map((g) => ({ id: g.id, title: g.title }));
  ...
  const weekOf = (dbSettings == null ? void 0 : dbSettings.week_of) || today;
  const habitResetDate = (dbSettings == null ? void 0 : dbSettings.habit_reset_date) || today;
  return {
    weekOf,
    goals,
    habits,
  ...
  try {
    ...
    for (const [monthKey, tasks] of Object.entries(data.completedTasks || {})) {
      for (const task of tasks) {
        ...
        fields.completed_at = (_a = task.completedAt) != null ? _a : today;
...
var lastKnownRemoteTaskIds = /* @__PURE__ */ new Set();
var lastKnownRemoteGoalIds = /* @__PURE__ */ new Set();
var lastKnownRemoteHabitIds = /* @__PURE__ */ new Set();
var realtimeChannel = null;
function subscribeToRealtime(onRemoteChange) {
  const supabase = getSupabase();
  const userId = getUserId();
  if (!supabase || !userId)
    return;
  unsubscribeFromRealtime();
  realtimeChannel = supabase.channel("focus-sync").on("postgres_changes", { event: "*", schema: "public", table: "tasks", filter: `user_id=eq.${userId}` }, () => {
    onRemoteChange();
...
var FocusPlugin = class extends import_obsidian6.Plugin {
  ...
  async onload() {
    ...
    setTimeout(() => void this.runAutoSort(), 3e3);
    ...
    try {
      ...
      if (this.settings.supabaseEmail && this.settings.supabasePassword) {
        ...
        if (error) {
          ...
          new import_obsidian6.Notice(`Focus cloud sync: Sign-in failed \u2014 ${error}`);
          return;
        }
      } else {
      ...
      const remoteHasTasks = remoteData && (Object.values(remoteData.tasks).some((arr) => arr.length > 0) || Object.keys(remoteData.completedTasks).length > 0);
      if (remoteHasTasks) {
        ...
        await this.saveTaskDataWithoutSync(data);
      }
      subscribeToRealtime(() => {
        this.onRemoteChange();
      });
   ...
   */
  onRemoteChange() {
    if (this.isSyncingFromRemote || this.overflowModalOpen)
      return;
    if (this.cloudSyncDebounce) {
      clearTimeout(this.cloudSyncDebounce);
    }
   ...
   */
  async pushLocalChanges(data) {
    if (!this.settings.cloudSyncEnabled || this.isSyncingFromRemote || !getUserId())
      return;
    try {
      await pushToRemote(data);
    } catch (err) {
   ...
   */
  async catchUpRemoteSync() {
    if (!this.settings.cloudSyncEnabled || !getUserId())
      return;
    this.isSyncingFromRemote = true;
    try {
      const remoteData = await pullFromRemote();
   ...
   */
  async saveTaskDataWithoutSync(data) {
   ...
   */
  setupTaskFileRenameWatcher() {
    this.registerEvent(
      this.app.vault.on("rename", (file, oldPath) => {
        if (!(file instanceof import_obsidian6.TFile))
          return;
        if ((0, import_obsidian6.normalizePath)(oldPath) !== (0, import_obsidian6.normalizePath)(this.settings.taskFilePath))
          return;
        ...
        updateLink.addEventListener("click", async (e) => {
          ...
          this.refreshFocusView();
          new import_obsidian6.Notice(`Task file path updated to: ${newPath}`);
        });
        fragment.appendChild(updateLink);
        fragment.appendText(` \u2192 ${newPath}`);
        new import_obsidian6.Notice(fragment, 1e4);
      })
    );
  }
  /**
  ...
  async addTask(title, section, url, doDate, doTime, recurrence) {
    const data = await this.loadTaskData();
    if (section === "immediate") {
      const activeImmediate = data.tasks.immediate.filter((t) => !t.completed);
   ...
   */
  async openFileWithoutDuplicate(file) {
...
var AutoSortOverflowModal = class extends import_obsidian6.Modal {
  constructor(plugin, _data, overflowTasks) {
    super(plugin.app);
    this.plugin = plugin;
  ...
  async renderContent(contentEl) {
    ...
    const overflowTasks = this.overflowTaskIds.map((id) => data.tasks.thisWeek.find((t) => t.id === id)).filter((t) => t != null);
    if (overflowTasks.length === 0) {
      this.close();
      return;
    }
    ...
    new import_obsidian6.Setting(contentEl).setName("Increase limit temporarily").setDesc(`Allow ${this.plugin.settings.maxImmediateTasks + overflowTasks.length} immediate tasks for now`).addButton(
      (btn) => btn.setButtonText("Increase limit").setCta().onClick(async () => {
    ...
    if (demotable.length > 0) {
      ...
      for (const task of demotable) {
        new import_obsidian6.Setting(contentEl).setName(task.title).addButton(
          (btn) => btn.setButtonText("Demote").onClick(async () => {
            const freshData = await this.plugin.loadTaskData();
    ...
    new import_obsidian6.Setting(contentEl).addButton(
      (btn) => btn.setButtonText("Dismiss").onClick(() => this.close())
    );
  }
  onClose() {
